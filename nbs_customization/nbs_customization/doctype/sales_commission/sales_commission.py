...
class SalesCommission(Document):
	...
	def calculate_row_amounts(self):
		...
		for row in self.commission_sales:
			...
			row.total_amount = flt(frappe.db.get_value("Sales Invoice", row.sale, "grand_total"))
	...
	def calculate_totals(self):
		...
		self.total_gross_commission = flt(sum(flt(r.gross_commission) for r in self.commission_sales), 2)
		self.total_withholding_tax_amount = flt(
			sum(flt(r.withholding_tax_amount) for r in self.commission_sales), 2
		)
		self.total_commission_payable = flt(sum(flt(r.commission_payable) for r in self.commission_sales), 2)
	...
	def _is_new_sale_row(self, row) -> bool:
		if self.is_new():
			return True
		return not frappe.db.exists("Commission Sale Entry", {"name": row.name, "parent": self.name})

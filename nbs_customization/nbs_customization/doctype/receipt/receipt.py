...
class Receipt(Document):
	...
	def _fetch_payment_row_details(self):
		...
		for row in self.receipt_payments:
			...
			if row.sales_invoice:
				invoice_date = frappe.db.get_value("Sales Invoice", row.sales_invoice, "posting_date")
				if invoice_date:
			...
			if row.payment_entry and row.sales_invoice:
				...
				if pe_ref:
					row.amount_due = flt(pe_ref.outstanding_amount) + flt(pe_ref.allocated_amount)
			...
			if row.payment_entry and not row.payment_method:
				mode = frappe.db.get_value("Payment Entry", row.payment_entry, "mode_of_payment")
				if mode:
					row.payment_method = mode

			# ---- receiving_account from PE header (if empty) -----------
			if row.payment_entry and not row.receiving_account:
				paid_to = frappe.db.get_value("Payment Entry", row.payment_entry, "paid_to")
				if paid_to:
	...
	def _compute_totals(self):
		...
		self.total_amount_received = sum(flt(row.amount_received) for row in self.receipt_payments)
		self.total_balance_due = sum(
			flt(row.balance_due) for row in self.receipt_payments if row.sales_invoice
		)

	def _compute_amount_in_words(self):
		currency = self.currency or frappe.db.get_value("Company", self.company, "default_currency")
		self.amount_in_words = money_in_words(self.total_amount_received, currency)
	...
	def _render_address_displays(self):
		if self.customer_address:
			self.customer_address_display = get_address_display(self.customer_address) or ""
		if self.billing_address:
			self.billing_address_display = get_address_display(self.billing_address) or ""
	...
	def _validate_minimum_rows(self):
		if not self.receipt_payments:
			frappe.throw(_("At least one Payment Entry row is required before submitting."))
		if self.total_amount_received <= 0:
			frappe.throw(_("Total Amount Received must be greater than zero before submitting."))
	...
	def _validate_no_duplicates(self):
		...
		for row in self.receipt_payments:
			...
			if key in seen:
				frappe.throw(
					_(
						"Row {0}: Payment Entry {1} / Invoice {2} appears more than once in this Receipt."
					).format(
						row.idx,
			...
			existing = frappe.db.get_value("Receipt Payment", filters, "parent")
			if existing:
				frappe.throw(
					_(
						"Row {0}: Payment Entry {1} for Invoice {2} is already covered by submitted Receipt {3}."
					).format(
						row.idx,
						frappe.bold(row.payment_entry),
						frappe.bold(row.sales_invoice or "—"),
	...
	def _update_payment_entries(self, action: str):
		...
		pe_names = {row.payment_entry for row in self.receipt_payments if row.payment_entry}
		for pe_name in pe_names:
			frappe.db.set_value("Payment Entry", pe_name, "custom_receipt", value)
...
def get_payment_entry_details(payment_entry: str) -> dict:
	...
	if pe.payment_type != "Receive":
		frappe.throw(
			_(
				"Only 'Receive' type Payment Entries are valid for customer receipts. {0} is of type '{1}'."
			).format(frappe.bold(payment_entry), pe.payment_type)
		)
	...
	for ref in pe.references:
		...
		invoice_date = frappe.db.get_value("Sales Invoice", ref.reference_name, "posting_date")
...
def _set_customer_addresses(doc):
	...
	if customer_address:
		...
		doc.customer_address_display = get_address_display(customer_address) or ""

	billing_address = get_default_address("Customer", doc.customer, sort_key="is_shipping_address")
	if not billing_address or billing_address == customer_address:
		...
		doc.billing_address_display = get_address_display(billing_address) or ""
...
def create_receipt_from_pe(source_name, target_doc=None):
	...
	if pe.docstatus != 1:
		frappe.throw(_("Payment Entry must be submitted before creating a Receipt."))
	if pe.payment_type != "Receive":
		frappe.throw(_("Only Receive-type Payment Entries are supported."))
	if pe.get("custom_receipt"):
		frappe.throw(
			_("Receipt {0} already exists for this Payment Entry.").format(frappe.bold(pe.custom_receipt))
		)

...
class PromissoryNote(Document):
	...
	def _sync_from_sales_order_and_deliveries(self):
		...
		self.customer_address = getattr(so, "customer_address", None) or frappe.db.get_value(
			"Customer", so.customer, "customer_primary_address"
		)
		self.shipping_address_name = getattr(so, "shipping_address_name", None) or self.customer_address
...
def recalculate_promissory_note_for_sales_order(sales_order: str):
	...
	try:
		...
		existing_items = frappe.db.get_all(
			"Promissory Note Item", filters={"parent": pn_name}, fields=["name", "item_code"]
		)
		...
		frappe.log_error(
			f"Failed to recalculate Promissory Note {pn_name} for Sales Order {sales_order}: {e!s}",
			"Promissory Note Recalculation Error",
		)
		frappe.msgprint(
			f"Promissory Note {pn_name} could not be recalculated. Check Error Log for details.",
			alert=True,
			indicator="orange",
		)

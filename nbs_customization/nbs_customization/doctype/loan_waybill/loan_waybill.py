...
class LoanWaybill(Document):
	...
	def validate(self):
		...
		if not self.company:
			frappe.throw(
				"Could not determine Company from Source Warehouse. Please check the warehouse configuration."
			)
	...
	def _validate_stock_availability(self):
		...
		for item in self.items:
			...
			if flt(actual_qty) < flt(item.quantity_loaned):
				frappe.throw(
					_(
						"Insufficient stock for Item {0} in Warehouse {1}. Available: {2}, Requested: {3}."
					).format(
						item.item_code,
	...
	def reverse_conversion(self, delivery_note_name, items):
		...
		for row in items:
			...
			if not bb:
				frappe.throw(
					_(
						"Cannot reverse: no Batch Balance row for Item {0}, Batch {1} in Loan Waybill {2}."
		...
		if history_name:
			frappe.delete_doc("Loan Conversion History", history_name, ignore_permissions=True, force=True)
	...
	def on_trash(self):
		...
		if self.stock_entry:
			...
			if se_status == 1:
				frappe.throw(
					_("Cannot delete Loan Waybill {0} — its Stock Entry {1} is still submitted.").format(
						self.name, self.stock_entry
					)
				)
...
def get_items_with_stock(doctype, txt, searchfield, start, page_len, filters):
	...
	return [(row[0], row[1], strip_html(row[2]), row[3]) for row in results]

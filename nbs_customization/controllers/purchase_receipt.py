"""
Overrides for the standard Purchase Receipt DocType.
Currently handles:
  - Denormalizing the linked Purchase Order(s) up to the PR header
...
def _collect_purchase_orders(doc):
    ...
    referenced across all PR items.  Order is preserved (first seen first).
    """
    seen = set()
    orders = []
...
def _set_custom_purchase_order(doc):
    """
    Populate custom_purchase_order on the PR header.

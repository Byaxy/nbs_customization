...
def make_promissory_note(source_name, target_doc=None, ignore_permissions=None):
	...
	def set_missing_values(source, target):
		...
		# Resolve addresses
		target.customer_address = getattr(source, "customer_address", None) or frappe.db.get_value(
			"Customer", source.customer, "customer_primary_address"
		)
		target.shipping_address_name = (
			getattr(source, "shipping_address_name", None) or target.customer_address
		)
		...
		if not target.items:
			frappe.throw(
				f"Sales Order {source_name} has been fully delivered. A Promissory Note is not required.",
...
def has_pending_loan_waybills(customer, sales_order):
	...
	items_with_so_remaining = tuple(
		r.item_code for r in so_qty_rows if flt(r.so_qty) - delivered_map.get(r.item_code, 0) > 0
	)
...
def make_delivery_note_from_loan(source_name: str, target_doc=None, ignore_permissions=None):
	...
	def condition(doc):
		...
		for selected in selected_items:
			if (
				selected.get("item_code") == doc.item_code
				and selected.get("batch_no") == doc.batch_no
				and selected.get("serial_no") == doc.serial_no
				and flt(selected.get("qty", 0)) > 0
			):
				return True
	...
	def postprocess_item(source, target, source_parent):
		...
		for selected in selected_items:
			if (
				selected.get("item_code") == source.item_code
				and selected.get("batch_no") == source.batch_no
				and selected.get("serial_no") == source.serial_no
				and flt(selected.get("qty", 0)) > 0
			):

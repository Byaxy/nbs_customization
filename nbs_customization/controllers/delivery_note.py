...
def validate_loan_source_warehouse(doc):
	...
	if loan.company:
		so_names = {
			item.against_sales_order for item in doc.items if getattr(item, "against_sales_order", None)
		}
	...
	for item in doc.items:
		if item.warehouse != loan.target_warehouse:
			frappe.throw(
				f"Row {item.idx}: Warehouse must be the customer loan warehouse ({loan.target_warehouse})."
			)
...
def _update_promissory_note_directly(doc):
	...
	try:
		...
		missing_so = [
			d.item_code for d in items if d.item_code and not getattr(d, "against_sales_order", None)
		]
		if missing_so:
			frappe.msgprint(
		...
		sales_orders = {
			getattr(d, "against_sales_order", None) for d in items if getattr(d, "against_sales_order", None)
		}
		...
		frappe.msgprint(
			"An error occurred while updating the Promissory Note. Check Error Log for details.",
			alert=True,
			indicator="orange",
		)
...
def _apply_loan_conversion(dn):
	...
	if not loan_waybill_name:
		frappe.throw(
			_(
				"Delivery Note {0} is marked as a Loan Conversion Waybill but has no Source Loan Waybill set."
			).format(dn.name)
		)
	...
	if loan_doc.docstatus != 1:
		frappe.throw(_("Source Loan Waybill {0} is not submitted.").format(loan_waybill_name))

	if loan_doc.conversion_status == "Fully Converted":
		frappe.throw(_("Source Loan Waybill {0} is already fully converted.").format(loan_waybill_name))
